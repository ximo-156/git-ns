		...
		<div>
			<scroll-view v-if="!isBigData" :scroll-y='true'
				...
				<div v-for='groupKey in Object.keys(dataFiltered)'
					...
					<div class="index-list__group-cell" v-for='item in dataFiltered[groupKey]' :clickable='true'
						:key="item.id">
						<div class="index-list__group-cell" @click="()=>{itemClickHandler(items)}"
							v-for="(items,index) in item.child" :key="items.id">
							{{items.name}}
						</div>
					</div>
				</div>
						...
						<div ref="bigDataTableDataPanel" class="index-list__big-data__table-data-panel">
							...
							<div v-for='groupKey in Object.keys(bigDataFiltered)' style='' :key='groupKey'>
								...
								<div class="index-list__group-cell" v-for='city in dataFiltered[groupKey]'
									...
									<div class="index-list__group-cell" v-for="(item,index) in city.child"
										:key="item.id">
										{{item.name}}
									</div>
								</div>
